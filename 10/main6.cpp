...
pthread_mutex_t database_access; // мьютекс для ограничения доступа к базе данных
pthread_mutex_t readers_changing; // мьютекс для смены количества читателей
pthread_mutex_t information_output; // мьютекс для вывода информации в консоль/файл
...
void *funcRead(void *param) {
    ...
    pthread_mutex_lock(&readers_changing);
    readers_count++;
    ...
    pthread_mutex_unlock(&readers_changing);

    // блокируем возможность записи в файл/консоль для конкурирующих потоков
    pthread_mutex_lock(&information_output);
    ...
    pthread_mutex_unlock(&information_output);
...
int main(int argc, char *argv[]) {
    ...
    pthread_mutex_init(&database_access, nullptr);
    pthread_mutex_init(&readers_changing, nullptr);
    pthread_mutex_init(&information_output, nullptr);
    ...
    // подчистка мьютексов
    pthread_mutex_destroy(&database_access);	
    pthread_mutex_destroy(&readers_changing);
    pthread_mutex_destroy(&information_output);
